...
// if ($start_date_alert && $end_date_alert) {
//     $query_alert = "SELECT mode, timestamp
//                     FROM system_history 
...
if ($start_date_alert && $end_date_alert) {
    ...
    $stmt_alert = $conn->prepare("
        SELECT sy.mode, sy.timestamp, u.fullname 
        FROM system_history sy 
        JOIN users u ON sy.user_id = u.id 
        WHERE sy.timestamp BETWEEN ? AND ? 
        ORDER BY sy.timestamp DESC 
        LIMIT ? OFFSET ?
    ");
